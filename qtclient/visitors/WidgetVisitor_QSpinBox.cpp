...
void WidgetVisitorState_QSpinBox::connectDataSignals( WidgetVisitorObject::DataSignalType dt, WidgetListener& listener)
{
	switch (dt)
	{
		...
		case WidgetVisitorObject::SigDoubleClicked:
			qCritical() << "try to connect to signal not provided" << m_spinBox->metaObject()->className() << WidgetVisitorObject::dataSignalTypeName(dt);
	}
}
