...
3. Dynamic Properties Used
 ...
 For dynamic properties to stear the application see section 'States and Behaviour of widgets' in WolfBook.
...
4.3. Refresh With Load Request
    (1) emit domain load request if defined

4.3. Refresh Without Load Request
    (1) save widget state
    (2) clear widget data
    (3) read globals
    (4) read assignments
	...
	:public VisitorInterface
{
	public:
		...
		QVariant valueAt( const QPoint& p) const;
